...
int OS_AddThread(void(*thread)(void)){
	if(numThreads == NUMTHREADS) return 0;
	...
	tcbs[numThreads].blocked = 0;
	tcbs[numThreads].sleep = 0;
	
	SetInitialStack(numThreads);
	Stacks[numThreads][STACKSIZE-2] = (int32_t)(thread);
	numThreads++;
	EndCritical(status);
	return 1;
}
...
int OS_AddThreads(void(*thread0)(void),
	...
	OS_AddThread(thread0);
	OS_AddThread(thread1);
	OS_AddThread(thread2);
	OS_AddThread(thread3);
	OS_AddThread(thread4);
	OS_AddThread(thread5);
